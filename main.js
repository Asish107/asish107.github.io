...
contactForm.addEventListener('submit', async (e) => {
  ...
  try {
    const res = await fetch('https://formspree.io/f/xnjlggky', {
      method: 'POST',
    ...
    if (res.ok) {
      ...
      note.textContent = '→ Got it! Asish will reply soon.';
    ...
    btn.textContent = 'Send Message';
    note.textContent = '→ Something went wrong. Try again or reach out on GitHub.';
    note.style.color = '#f87171';
  }
});

/* ─── GITHUB API — live public repo count ─────────────────────────── */
(async function fetchGitHubStats() {
  const repoEl    = document.getElementById('stat-repos');
  if (!repoEl) return;

  try {
    const res  = await fetch('https://api.github.com/users/Asish107', {
      headers: { 'Accept': 'application/vnd.github.v3+json' }
    });
    if (!res.ok) return; // silently fail — fallback value stays

    const data = await res.json();
    const count = data.public_repos ?? 2;

    // Update the data-target so the counter animation uses the live number
    repoEl.dataset.target = count;

    // If the counter has already animated (section was visible on load),
    // update the displayed number directly too
    if (repoEl.textContent !== '0') {
      animateCounter(repoEl, count);
    }
  } catch (_) {
    // Network error — keep the hardcoded fallback, no console noise
  }
})();
